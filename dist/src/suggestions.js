import { DEFAULT_HOST } from "./host.js";
...
function ownerFlag(ctx) {
    return ctx.owner ? ` --owner ${ctx.owner}` : "";
}
function normalizeRepoFlagLine(line) {
    return line.replace(/`gh-axi -R ([^`\s]+) ([^`]+)`/g, "`gh-axi $2 -R $1`");
}
let activeHost;
export async function withSuggestionHost(host, callback) {
...
function appendHostnameFlag(line, ctx) {
...
const table = [
    ...
    {
        match: (c) => c.domain === "project" && ["item-add", "item-create"].includes(c.action),
        lines: (c) => [
    ...
    {
        match: (c) => c.domain === "secret" && c.action === "list" && c.isEmpty === true,
        lines: (c) => [
    ...
    {
        match: (c) => c.domain === "variable" && c.action === "list" && c.isEmpty === true,
        lines: (c) => [
    ...
    {
        match: (c) => c.domain === "gist" && c.action === "list" && c.isEmpty === true,

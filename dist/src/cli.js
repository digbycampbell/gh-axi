    ...
    await runAxiCli({
        ...
        formatError: (error) => {
            ...
            return {
                ...
                exitCode: error instanceof StackError
                    ? error.exitCode
...
function withRepoContext(command, handler) {
    return (args, ctx) => withSuggestionHost(ctx?.host, () => handler(parseRepoContextArgs(command, args).strippedArgs, repoContext(ctx)));
}
function withLocalRepoContext(handler) {
    return (args, ctx) => {
        const parsed = parseRepoContextArgs("stack", args);
        if (parsed.repoFlag !== undefined || process.env["GH_REPO"]) {
            throw new AxiError("stack commands operate on the repository in the current working directory and do not support -R, --repo, or GH_REPO", "VALIDATION_ERROR");
        }
        return withSuggestionHost(ctx?.host, () => handler(parsed.strippedArgs));
    };
}
function repoContext(ctx) {
    return ctx && "nwo" in ctx ? ctx : undefined;
}
function resolveHostContext(hostFlag) {
    if (hostFlag === undefined) {
        return undefined;
    }

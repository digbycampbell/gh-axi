...
import { field, custom, relativeTime, pluck, renderList, renderDetail, renderHelp, renderOutput, renderError, } from "../toon.js";
...
const defaultSchema = [
    field("id"),
    field("description"),
    custom("files", (item) => Object.keys(item["files"] ?? {}).length),
 ...
 */
function rejectUnknownFlags(tokens, allowed) {
    const unknown = tokens.filter((a) => a.startsWith("-") && !allowed.includes(a));
    if (unknown.length > 0) {
        throw new AxiError(`Unknown flag(s): ${unknown.join(", ")}`, "VALIDATION_ERROR");
    }
}
/**
 ...
 */
async function readRequiredStdin(example) {
    const content = await readStdin();
    if (content.length === 0) {
        throw new AxiError("no content received on stdin; nothing was piped", "VALIDATION_ERROR", [example]);
    }
    return content;
}
// ---------------------------------------------------------------------------
 ...
 */
function truncateGistContent(content, maxLen, full) {
    if (full || content.length <= maxLen)
        return content;
...
async function viewGist(args) {
    // Reject -w/--web up-front before consuming any other args (AXI P6).
    if (args.some((a) => flagName(a) === "-w" || flagName(a) === "--web")) {
        throw new AxiError("-w/--web is not supported: opening a browser is a no-op in agent contexts", "VALIDATION_ERROR");
    }
    ...
    if (!selector) {
        throw new AxiError("gist view requires a gist id or URL", "VALIDATION_ERROR", ["Usage: gh-axi gist view <id|url>"]);
    }
    ...
    const data = await ghJson(["api", `/gists/${id}`]);
    ...
    if (filenameArg !== undefined) {
        ...
        if (!file) {
            ...
            throw new AxiError(`File "${filenameArg}" not found in gist ${id}. Available: ${available || "(none)"}`, "VALIDATION_ERROR");
        }
        return renderOutput([
    ...
    return renderOutput([
        renderDetail("gist", data, gistMetaSchema),
...
async function listGists(args) {
    ...
    if (wantPublic && wantSecret) {
        throw new AxiError("--public and --secret are mutually exclusive", "VALIDATION_ERROR");
    }
    ...
    if (limitArg !== undefined) {
        ...
        if (isNaN(n) || n < 1) {
            throw new AxiError(`--limit must be a positive integer, got: ${limitArg}`, "VALIDATION_ERROR");
        }
        limit = n;
    }
    else {
    ...
    const apiArgs = ["api", `/gists?per_page=${perPage}`];
    if (paginate) {
    ...
    const gists = await ghJson(apiArgs);
...
async function deleteGist(args) {
    ...
    if (!selector)
        throw new AxiError("Gist is required: gh-axi gist delete <id|url>", "VALIDATION_ERROR");
    if (extra)
...
async function cloneGist(args) {
    ...
    const extra = positionals[2];
    if (!selector)
        throw new AxiError("Gist is required: gh-axi gist clone <id|url>", "VALIDATION_ERROR");
    if (extra)
        throw new AxiError(`Unexpected argument: ${extra}`, "VALIDATION_ERROR");
...
async function createGist(args) {
    ...
    if (!wantPublic && !wantSecret) {
        throw new AxiError("gist create requires --public or --secret; neither was given\n" +
            "A secret gist is unlisted (anyone with the URL can read it), not private.", "VALIDATION_ERROR");
    }
    ...
    if (positionals.length > 0 && fileFlags.length > 0) {
        throw new AxiError("Cannot mix positional paths with --file; use one form: " +
            "either `gist create a.py b.py` or `gist create --file a.py --file b.py`", "VALIDATION_ERROR");
    }
    ...
    if (hasFileArgs && filename !== undefined) {
        throw new AxiError("Cannot mix file paths with --filename; use one input form", "VALIDATION_ERROR");
    }
    // At least one input source must be provided.
    if (!hasFileArgs && filename === undefined) {
        throw new AxiError("gist create requires at least one file: pass positional path(s), " +
            "--file <path>, or pipe content with --filename <name>", "VALIDATION_ERROR");
    }
    ...
    if (filename !== undefined) {
        ...
        if (isStdinTTY()) {
            throw new AxiError("--filename requires piped content on stdin; no pipe was detected", "VALIDATION_ERROR", [`echo 'content' | gh-axi gist create --filename <name> --public`]);
        }
        const content = await readRequiredStdin(`echo 'content' | gh-axi gist create --filename <name> --public`);
    ...
    if (visibility === "secret") {
        helpLines.push("a secret gist is unlisted, not private — anyone with the URL can read it");
    }
...
async function editGist(args) {
    ...
    if (!id) {
        throw new AxiError("Gist ID or URL is required: gh-axi gist edit <id|url> [flags]", "VALIDATION_ERROR");
    }
    if (positionals.length > 1) {
        throw new AxiError(`Unexpected argument: ${positionals[1]}`, "VALIDATION_ERROR");
    }
    // Reject mutually exclusive file operations in a single call.
    const fileOpCount = [filenameFlag, addFlag, removeFlag].filter((v) => v !== undefined).length;
    if (fileOpCount > 1) {
        throw new AxiError("--filename, --add, and --remove are mutually exclusive; pass only one per invocation", "VALIDATION_ERROR");
    }
    ...
    if (filenameFlag === undefined &&
        ...
        descFlag === undefined) {
        throw new AxiError("nothing to edit; pass --filename <name> with piped content, --add <path|name>, --remove <name>, or --desc <text>", "VALIDATION_ERROR");
    }
    ...
    if (wantsStdin && filenameFlag === undefined && addFlag === undefined) {
        throw new AxiError("stdin content (-) requires --filename <name> or --add <name> to identify the target file", "VALIDATION_ERROR", [
    ...
    if (filenameFlag !== undefined) {
        ...
        if (isStdinTTY()) {
            throw new AxiError("--filename requires content piped via stdin", "VALIDATION_ERROR", [
        ...
        if (descFlag !== undefined)
            ghArgs.push("--desc", descFlag);
        const content = await readRequiredStdin("Example: echo 'content' | gh-axi gist edit <id|url> --filename <name>");
        ...
        if (isStdinTTY()) {
            throw new AxiError("--add with the stdin sentinel (-) requires content piped via stdin", "VALIDATION_ERROR", ["Example: echo 'content' | gh-axi gist edit <id|url> --add <name> -"]);
        }
        const ghArgs = ["gist", "edit", id, "--add", addFlag, "-"];
        if (descFlag !== undefined)
            ghArgs.push("--desc", descFlag);
        const content = await readRequiredStdin("Example: echo 'content' | gh-axi gist edit <id|url> --add <name> -");
        await ghExecWithStdin(ghArgs, content);
    }
        ...
        if (addFlag !== undefined)
            ghArgs.push("--add", addFlag);
        if (removeFlag !== undefined)
            ghArgs.push("--remove", removeFlag);
        if (descFlag !== undefined)
            ghArgs.push("--desc", descFlag);
        await ghExec(ghArgs);
    }
...
async function renameGist(args) {
    ...
    if (unknownFlags.length > 0) {
        throw new AxiError(`gist rename takes no flags; unexpected: ${unknownFlags.join(", ")}`, "VALIDATION_ERROR");
    }
    const positionals = allAfterSub;
    if (positionals.length < 3) {
        throw new AxiError("usage: gh-axi gist rename <id|url> <old> <new>", "VALIDATION_ERROR");
    }
    if (positionals.length > 3) {
        throw new AxiError(`too many arguments; expected: gh-axi gist rename <id|url> <old> <new>`, "VALIDATION_ERROR");
    }
    const [id, oldName, newName] = positionals;
...
// See AGENTS.md "User-scoped commands" section.
export async function gistCommand(args) {
    const sub = args[0];

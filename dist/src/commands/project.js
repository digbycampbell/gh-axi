...
import { field, pluck, custom, renderList, renderDetail, renderHelp, renderError, renderOutput, } from "../toon.js";
// ---------------------------------------------------------------------------
// Help
// ---------------------------------------------------------------------------
const PROJECT_FLAGS = {
...
// Helpers
// ---------------------------------------------------------------------------
function resolveOwner(args, ctx) {
...
function takeProjectNumber(args) {
    for (let index = 0; index < args.length; index++) {
        const arg = args[index];
        if (arg.startsWith("--")) {
            if (!arg.includes("=") && PROJECT_NUMBER_VALUE_FLAGS.has(arg))
                index++;
            continue;
        }
        if (/^\d+$/.test(arg)) {
...
function renderProjectItemValue(value) {
...
function setProjectItemField(row, key, value) {
    ...
    if (String(row[key]) !== String(rendered))
        row[`${key}_field`] = rendered;
}
...
function renderProjectItems(items) {
    const rows = items.map((item) => {
        ...
        if (content.number !== undefined)
            row.number = content.number;
        if (content.repository !== undefined)
            row.repository = content.repository;
        ...
        if (typeof content.body === "string" && content.body)
            row.body = truncateBody(content.body, 300, PROJECT_ITEM_BODY_TRUNCATION_OPTIONS);
        if (content.url !== undefined)
            row.url = content.url;
        for (const [key, value] of Object.entries(item)) {
            if (key === "content")
                continue;
            if (hasNestedContent && key === "id")
                continue;
            if (!hasNestedContent && CONTENT_ITEM_KEYS.has(key))
                continue;
...
function renderProjectFields(fields) {
...
const projectListSchema = [
    field("number"),
    field("title"),
    custom("state", (item) => (item.closed ? "closed" : "open")),
...
const projectViewSchema = [
    ...
    field("title"),
    custom("state", (item) => (item.closed ? "closed" : "open")),
    custom("visibility", (item) => item.public ? "public" : "private"),
    pluck("owner", "login", "owner"),
...
// Subcommands
// ---------------------------------------------------------------------------
async function projectList(args, ctx) {
    ...
    if (closed)
        ghArgs.push("--closed");
    const result = await ghJson(ghArgs);
    ...
    return renderOutput([
        ...
        renderHelp(getSuggestions({ domain: "project", action: "list", isEmpty, owner })),
    ]);
}
async function projectView(args, ctx) {
    const owner = resolveOwner(args, ctx);
    ...
    const project = await ghJson(ghArgs);
    return renderOutput([renderDetail("project", project, projectViewSchema)]);
}
async function projectItemList(args, ctx) {
    const owner = resolveOwner(args, ctx);
    const num = takeProjectNumber(args);
...
async function projectFieldList(args, ctx) {
    const owner = resolveOwner(args, ctx);
    const num = takeProjectNumber(args);
    const limit = takeFlag(args, "--limit") ?? "30";
    const ghArgs = [
...
async function projectItemAdd(args, ctx) {
    const owner = resolveOwner(args, ctx);
    const num = takeProjectNumber(args);
    const url = takeFlag(args, "--url");
    if (!url)
        throw new AxiError("--url is required: gh-axi project item-add <number> --url <issue-or-pr-url>", "VALIDATION_ERROR");
    const ghArgs = [
        "project",
        "item-add",
        String(num),
    ...
    const item = await ghJson(ghArgs);
    return renderOutput([
        renderDetail("added", { id: item.id ?? null, title: item.title ?? null, status: "ok" }, [field("id"), field("title"), field("status")]),
        renderHelp(getSuggestions({ domain: "project", action: "item-add", id: num, owner })),
    ]);
}
async function projectItemCreate(args, ctx) {
    const owner = resolveOwner(args, ctx);
    const num = takeProjectNumber(args);
    const title = takeFlag(args, "--title");
    if (!title)
        throw new AxiError('--title is required: gh-axi project item-create <number> --title "..."', "VALIDATION_ERROR");
    ...
    const item = await ghJson(ghArgs);
    return renderOutput([
        renderDetail("created", { id: item.id ?? null, title: item.title ?? title, status: "ok" }, [field("id"), field("title"), field("status")]),
        renderHelp(getSuggestions({
            domain: "project",
            action: "item-create",
            id: num,
...
async function projectItemEdit(args) {
    const id = takeFlag(args, "--id");
    if (!id)
        throw new AxiError('--id is required: gh-axi project item-edit --id <item-id> [--field-id <id> --project-id <id> --text "..."]', "VALIDATION_ERROR");
    ...
    if (projectId)
        ghArgs.push("--project-id", projectId);
    if (fieldId)
        ghArgs.push("--field-id", fieldId);
    if (text !== undefined)
        ghArgs.push("--text", text);
    if (numberValue !== undefined)
        ghArgs.push("--number", numberValue);
    if (date)
        ghArgs.push("--date", date);
    if (singleSelectOptionId)
        ghArgs.push("--single-select-option-id", singleSelectOptionId);
    if (iterationId)
        ghArgs.push("--iteration-id", iterationId);
    if (title)
...
async function projectItemArchive(args, ctx) {
    const owner = resolveOwner(args, ctx);
    const num = takeProjectNumber(args);
    const id = takeFlag(args, "--id");
    if (!id)
        throw new AxiError("--id is required: gh-axi project item-archive <number> --id <item-id>", "VALIDATION_ERROR");
    ...
    const item = await ghJson(ghArgs);
    return renderOutput([
        renderDetail(undo ? "unarchived" : "archived", { id: item.id ?? id, status: "ok" }, [field("id"), field("status")]),
        renderHelp(getSuggestions({
            domain: "project",
            action: "item-archive",
            id: num,
...
async function projectItemDelete(args, ctx) {
    const owner = resolveOwner(args, ctx);
    const num = takeProjectNumber(args);
    const id = takeFlag(args, "--id");
    if (!id)
        throw new AxiError("--id is required: gh-axi project item-delete <number> --id <item-id>", "VALIDATION_ERROR");
    const ghArgs = [
        "project",
        "item-delete",
        String(num),
...
async function projectCreate(args, ctx) {
    const owner = resolveOwner(args, ctx);
    const title = takeFlag(args, "--title");
    if (!title)
        throw new AxiError('--title is required: gh-axi project create --title "..."', "VALIDATION_ERROR");
    const ghArgs = [
        "project",
        "create",
        "--title",
    ...
    const project = await ghJson(ghArgs);
    return renderOutput([
        renderDetail("created", { number: project.number ?? null, url: project.url ?? null }, [field("number"), field("url")]),
        renderHelp(getSuggestions({
            domain: "project",
            action: "create",
            id: project.number,
...
async function projectEdit(args, ctx) {
    const owner = resolveOwner(args, ctx);
    const num = takeProjectNumber(args);
    const title = takeFlag(args, "--title");
    ...
    return renderOutput([
        ...
        renderHelp(getSuggestions({ domain: "project", action: "edit", id: num, owner })),
    ]);
}
async function projectClose(args, ctx) {
    const owner = resolveOwner(args, ctx);
    const num = takeProjectNumber(args);
    const undo = takeBoolFlag(args, "--undo");
    ...
    if (alreadyDesired) {
        return renderOutput([
            renderDetail("project", { number: num, state: undo ? "open" : "closed", already: true }, [field("number"), field("state"), field("already")]),
            renderHelp(getSuggestions({ domain: "project", action: "close", id: num, owner })),
        ]);
    }
    const ghArgs = [
        "project",
...
async function projectCopy(args, ctx) {
    const num = takeProjectNumber(args);
    const sourceOwner = takeFlag(args, "--source-owner") ?? ctx?.owner ?? "@me";
    const targetOwner = takeFlag(args, "--target-owner");
    if (!targetOwner)
        throw new AxiError('--target-owner is required: gh-axi project copy <number> --target-owner <login> --title "..."', "VALIDATION_ERROR");
    const title = takeFlag(args, "--title");
    if (!title)
        throw new AxiError('--title is required: gh-axi project copy <number> --target-owner <login> --title "..."', "VALIDATION_ERROR");
    ...
    const project = await ghJson(ghArgs);
    return renderOutput([
        renderDetail("copied", { number: project.number ?? null, url: project.url ?? null }, [field("number"), field("url")]),
        renderHelp(getSuggestions({
            domain: "project",
            action: "copy",
            id: project.number,
...
// Router
// ---------------------------------------------------------------------------
export async function projectCommand(args, ctx) {
    const sub = args[0];
    ...
    switch (sub) {
        ...
        case undefined:
            return PROJECT_HELP;
        default:
            return renderError(`Unknown project subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `gh-axi project --help` to see available subcommands"]);
    }
}

...
function parseArgs(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (name !== METHOD_FLAG &&
            ...
            throw new AxiError(`unknown flag ${name} for gh-axi api. Supported flags: ${SUPPORTED_FLAGS.join(", ")}`, "VALIDATION_ERROR");
        }
        ...
        if (name === METHOD_FLAG) {
            ...
            if (!HTTP_METHODS.has(upper)) {
                throw new AxiError(`${name} ${value} is not a supported HTTP method. Supported: ${[...HTTP_METHODS].join(", ")}`, "VALIDATION_ERROR");
            }
            parsed.method = upper;
        }
        else if (name === "--field") {
...
export async function apiCommand(args, ctx) {
    if (args[0] === "--help" || args.length === 0)
        return API_HELP;
    const { positionals, fields, headers, jq, template, method: methodFlag, input, paginate, full, } = parseArgs(args);
    const pathRequired = new AxiError("API path is required: gh-axi api [<method>] <path>", "VALIDATION_ERROR");
    ...
    if (methodFlag !== undefined && methodGiven) {
        throw new AxiError("method given twice: use either -X <method> or the positional method, not both", "VALIDATION_ERROR");
    }
    if (positionals.length > (methodGiven ? 2 : 1)) {
        throw new AxiError("too many arguments for gh-axi api: expected [<method>] <path>", "VALIDATION_ERROR");
    }
    if (methodGiven && positionals.length < 2)
        throw pathRequired;
    ...
    if (input === "-") {
        if (isStdinTTY())
            throw new AxiError("--input - requires piped stdin (no request body to read)", "VALIDATION_ERROR");
    ...
    try {
        ...
        if (truncated) {
            result.api_response.original_length =
                trimmed.length;
        }
...
function truncateString(value) {
    if (value.length <= STRING_VALUE_TRUNCATION_LIMIT)
        return value;
...
function clampString(value) {
    if (value.length <= LONG_STRING_CLEANUP_THRESHOLD)
        return value;
 ...
 */
function shapeOutput(obj, stripNoisyKeys, truncateValues, depth = 0) {
    if (depth > 8)
        return obj;
    if (Array.isArray(obj)) {
        return obj.map((item) => shapeOutput(item, stripNoisyKeys, truncateValues, depth + 1));
    }

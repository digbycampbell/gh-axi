...
import { field, lower, renderList, renderDetail, renderHelp, renderOutput, renderError, } from '../toon.js';
...
async function findWorkflow(id, ctx) {
    const workflows = await ghJson(['workflow', 'list', '--json', 'id,name,state,path', '--all'], ctx);
    const match = workflows.find((w) => String(w.id) === id || w.name === id || (typeof w.name === 'string' && w.name.toLowerCase() === id.toLowerCase()));
    if (!match)
        throw new AxiError(`Workflow "${id}" not found`, 'NOT_FOUND');
    return match;
}
async function listWorkflows(args, ctx) {
    ...
    if (hasFlag(args, '--all'))
        ghArgs.push('--all');
    const workflows = await ghJson(ghArgs, ctx);
...
async function viewWorkflow(args, ctx) {
    const positionals = args.filter((a) => !a.startsWith('--'));
    const id = positionals[1];
    if (!id)
        throw new AxiError('Workflow ID or name is required: gh-axi workflow view <id|name>', 'VALIDATION_ERROR');
    const match = await findWorkflow(id, ctx);
    return renderOutput([renderDetail('workflow', match, viewSchema)]);
}
async function runWorkflow(args, ctx) {
    const positionals = args.filter((a) => !a.startsWith('--'));
    const id = positionals[1];
    if (!id)
        throw new AxiError('Workflow ID or name is required: gh-axi workflow run <id|name>', 'VALIDATION_ERROR');
...
async function enableWorkflow(args, ctx) {
    const positionals = args.filter((a) => !a.startsWith('--'));
    const id = positionals[1];
    if (!id)
        throw new AxiError('Workflow ID or name is required: gh-axi workflow enable <id|name>', 'VALIDATION_ERROR');
...
async function disableWorkflow(args, ctx) {
    const positionals = args.filter((a) => !a.startsWith('--'));
    const id = positionals[1];
    if (!id)
        throw new AxiError('Workflow ID or name is required: gh-axi workflow disable <id|name>', 'VALIDATION_ERROR');
...
export async function workflowCommand(args, ctx) {
    const sub = args[0];

...
const REMOTE_FLAGS = { "--remote": "value" };
...
const FAILURE_DIAGNOSTIC = /failed|could not|couldn't|cannot create|not .*updated automatically|remain stacked|not fully|skipping rebase/i;
...
function unexpectedOutput(stdout) {
    return new AxiError(`Unexpected gh stack output: ${stdout.slice(0, 200)}`, "UNKNOWN");
}
async function runStack(action, args) {
    ...
    const aborted = stderrDiagnostics.some(({ text }) => /sync aborted/i.test(text));
...
async function execute(args) {
    ...
    throw new StackError(message, result.exitCode, stackRecovery(args[1] ?? "", result.exitCode));
}
function stackRecovery(action, exitCode) {
    switch (exitCode) {
        case 2:
...
function validateAdd(args) {
    const normalized = args.flatMap((arg) => arg === "-Am" ? ["-A", "-m"] : arg === "-um" ? ["-u", "-m"] : [arg]);
    const positional = parseArgs(normalized, {
        "-m": "value",
        "--message": "value",
        "-A": "boolean",
        "--all": "boolean",
    ...
    const hasMessage = normalized.some((arg) => arg === "-m" || arg === "--message" || arg.startsWith("--message="));
    const stagesAll = normalized.some((arg) => arg === "-A" || arg === "--all");
    const stagesTracked = normalized.some((arg) => arg === "-u" || arg === "--update");
...
function validateMerge(args) {
    ...
    const methods = args.filter((arg) => ["--merge", "--squash", "--rebase", "--merge-method"].includes(arg) ||
        arg.startsWith("--merge-method="));
    if (methods.length > 1)
        throw validation("Choose only one merge method");
}
function parseArgs(args, flags, minPositionals, maxPositionals = Number.POSITIVE_INFINITY) {
    const positional = [];
    for (let index = 0; index < args.length; index++) {
        ...
        if (!kind)
            throw validation(`Unsupported stack flag: ${name}`);
        if (kind === "boolean") {
    ...
    if (positional.length < minPositionals) {
        throw validation(`Expected at least ${minPositionals} stack argument${minPositionals === 1 ? "" : "s"}`);
    }
    if (positional.length > maxPositionals) {
        throw validation(`Expected at most ${maxPositionals} stack argument${maxPositionals === 1 ? "" : "s"}`);
    }
    return positional;
}
...
function isFailureDiagnostic({ marker, text }) {

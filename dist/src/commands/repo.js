...
async function viewRepo(args, ctx) {
    ...
    if (repoArg && ctx?.source === 'flag') {
        throw new AxiError(`Unsupported positional argument for repo view with --repo: ${repoArg}. Use --repo <owner/name> to select a repository.`, 'VALIDATION_ERROR');
    }
    if (extraArg) {
        throw new AxiError(`Unsupported positional argument for repo view: ${extraArg}. Use --repo <owner/name> to select a repository.`, 'VALIDATION_ERROR');
    }

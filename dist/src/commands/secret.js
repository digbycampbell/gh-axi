...
import { field, relativeTime, renderList, renderHelp, renderOutput, renderError, } from "../toon.js";
...
function matchUnsupportedScopeFlag(tok) {
 ...
 */
function resolveScope(tokens, sub) {
    const positionals = [];
    let env;
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (unsupported)
            throw unsupportedScopeError(unsupported, sub);
...
function malformedEnvError(sub) {
    return new AxiError("--env requires an environment name", "VALIDATION_ERROR", [usageFor(sub)]);
}
function conflictingEnvError(sub) {
    return new AxiError("conflicting --env flags: specify a single environment", "VALIDATION_ERROR", [usageFor(sub)]);
}
function unsupportedScopeError(flag, sub) {
    return new AxiError(`gh-axi secret supports only repository scope and --env (environment) scope; ${flag} is not supported`, "VALIDATION_ERROR", [usageFor(sub)]);
}
function unknownFlagError(tok, sub) {
    // Echo the flag name only, never an attached `=value`, so a value cannot leak.
    const flagName = tok.split("=", 1)[0];
    return new AxiError(`unknown flag for gh-axi secret ${sub}: ${flagName}`, "VALIDATION_ERROR", [usageFor(sub), "gh-axi secret --help"]);
}
function hasBodyFlag(args) {
    return args.some((arg) => arg === "--body" ||
        arg.startsWith("--body=") ||
        arg === "-b" ||
        arg.startsWith("-b="));
}
async function listSecrets(args, ctx) {
    ...
    const secrets = await ghJson(["secret", "list", "--json", "name,updatedAt", ...scopeArgs], ctx);
...
async function setSecret(args, ctx) {
    const remaining = args.slice(1);
    if (hasBodyFlag(remaining)) {
        throw new AxiError("Secret values must be piped via stdin; --body/-b is not accepted because flags are visible in process argv", "VALIDATION_ERROR", [`echo -n "<value>" | gh-axi secret set <name>`]);
    }
...
async function deleteSecret(args, ctx) {
...
export async function secretCommand(args, ctx) {
    const sub = args[0];

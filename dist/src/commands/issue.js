...
import { hasFlag, getFlag, getAllFlags, pushRepeated, getPositional, requireNumber, takeFlag, takeBoolFlag, rejectUnknownFlags, } from "../args.js";
import { takeBody, truncateBody } from "../body.js";
import { parseFields } from "../fields.js";
import { formatCountLine } from "../format.js";
import { fetchListTotal, isSearchableMilestone, } from "../totals.js";
...
const withFullBody = (schema) => schema.map((f) => "as" in f && f.as === "body"
    ? custom("body", (item) => typeof item.body === "string" ? item.body : "")
    : f);
const viewSchemaFull = withFullBody(viewSchema);
const viewSchemaFullWithoutType = withFullBody(viewSchemaWithoutType);
...
const commentResultSchemaFull = withFullBody(commentResultSchema);
...
async function listIssues(args, ctx) {
    ...
    if (items.length === limit && ctx && countable) {
        ...
        if (milestone)
            filters.push({ key: "milestone", value: milestone });
...
async function viewIssue(args, ctx) {
    ...
    if (withComments && Array.isArray(item.comments)) {
        ...
        blocks.push(renderList("comments", item.comments, commentSchema.filter((d) => ("key" in d ? d.key !== "number" : true))));
    }
    return renderOutput(blocks);
}
...
export async function issueCommand(args, ctx) {
    const sub = args[0];
    if (sub === "subissue") {
        rejectUnknownFlags(args.slice(1), ISSUE_FLAGS.subissue, "issue", "subissue");
    ...
    switch (sub) {
        ...
        case "comment":
            rejectUnknownFlags(args.slice(1), ISSUE_FLAGS.comment, "issue", "comment");
        ...
        case "transfer":
            rejectUnknownFlags(args.slice(1), ISSUE_FLAGS.transfer, "issue", "transfer");

...
const SEARCH_TOTAL_QUERY = "query($q: String!) { search(query: $q, type: ISSUE) { issueCount } }";
/**
 ...
 */
export function searchQualifier(key, value) {
 ...
 */
export function searchQualifiers(key, value) {
    return value
        .split(",")
        .map((part) => part.trim())
...
export function stateQualifiers(state) {
 ...
 */
export function isSearchableMilestone(value) {
    return !/^\d+$/.test(value);
}
/**
 ...
 */
export function isSearchableValue(value) {
 ...
 */
export async function fetchSearchTotal(qualifiers) {
    try {
 ...
 */
export async function fetchRepositoryTotal(ctx, entity, state) {
    try {
 ...
 */
function statesArgument(entity, state) {
    ...
    const states = entity === "pullRequests" && normalized === "CLOSED"
        ? "CLOSED,MERGED"
        : normalized;
 ...
 */
export async function fetchListTotal(ctx, entity, state, filters) {
    if (!filters.every((filter) => isSearchableValue(filter.value))) {
        return undefined;
    }
    if (filters.length === 0)
        return fetchRepositoryTotal(ctx, entity, state);
    const qualifiers = filters.flatMap((filter) => filter.list

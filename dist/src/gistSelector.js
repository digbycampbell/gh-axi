 ...
 */
export function gistIdFromSelector(selector) {
    const trimmed = selector.trim();
    if (!trimmed) {
        throw new AxiError("Gist selector must not be empty", "VALIDATION_ERROR");
    }
    if (/\s/.test(trimmed)) {
        throw new AxiError(`Gist selector must not contain whitespace: "${selector.trim()}"`, "VALIDATION_ERROR");
    }
...
function extractIdFromUrl(rawUrl) {
    ...
    if (!validHosts.has(hostname)) {
        throw new AxiError(`Gist URL host "${hostname}" does not match the configured host "${configured}"`, "VALIDATION_ERROR");
    }
    // Take the last non-empty path segment — correct for all URL shapes.
    const segments = url.pathname.split("/").filter(Boolean);
    const id = segments[segments.length - 1];
    if (!id) {
        throw new AxiError(`Could not extract a gist id from URL: ${rawUrl}`, "VALIDATION_ERROR");
    }
...
//# sourceMappingURL=gistSelector.js.map